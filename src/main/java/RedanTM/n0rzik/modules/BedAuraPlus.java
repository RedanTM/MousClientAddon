...
import RedanTM.n0rzik.util.BedAura.Bed;
import RedanTM.n0rzik.util.BedAura.ItemHelper;
...
public class BedAuraPlus extends MousModule {
    ...
    public BedAuraPlus() {
        super(Mous.CATEGORY, "bed-aura-plus", "The famous troll module.");
    }

    @Override
    public void onActivate() {
        target = null;
        ogItem = HelperSurround.getItemFromSlot(autoMoveSlot.get() - 1);
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (preventEscape.get() && BlockHelper.getBlock(target.getBlockPos().up(2)) != Blocks.OBSIDIAN && HelperSurround.isInHole(target)) {
        ...
        if (breakSelfTrap.get() && shouldTrapMine()) {
            ...
            if (pick.found()) {
                HelperSurround.updateSlot(pick.slot());
                info("Breaking " + target.getEntityName() + "'s self-trap.");
                stb = Bed.getSelfTrapBlock(target, preventEscape.get());
                Automation.doPacketMine(stb);
                sentTrapMine = true;
                return;
            }
        }
        if (placePos == null && Automation.isBurrowed(target, false) && breakBurrow.get() && !sentBurrowMine) {
            FindItemResult pick = ItemHelper.findPick();
            if (pick.found()) {
                HelperSurround.updateSlot(pick.slot());
                info("Breaking " + target.getEntityName() + "'s burrow.");
                Automation.doPacketMine(target.getBlockPos());
                sentBurrowMine = true;
                return;
            }
        }
        if (placePos == null && Automation.isWebbed(target) && breakWeb.get()) {
            FindItemResult sword = ItemHelper.findSword();
            if (sword.found()) {
                HelperSurround.updateSlot(sword.slot());
                if (webTimer <= 0) {
                ...
                Automation.mineWeb(target, sword.slot());
                return;
            }
        }

        if (sentTrapMine && didTrapMine()) { sentTrapMine = false; stb = null; }
        if (sentBurrowMine && !Automation.isBurrowed(target, false)) sentBurrowMine = false;
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (renderAutomation.get() && target != null) {
            ...
            if (Automation.isWeb(target.getBlockPos())) event.renderer.box(target.getBlockPos(), sideColor.get(), lineColor.get(), shapeMode.get(), 0);
            if (Automation.isWeb(target.getBlockPos().up())) event.renderer.box(target.getBlockPos().up(), sideColor.get(), lineColor.get(), shapeMode.get(), 0);
        }
    }

    private boolean shouldTrapMine() {
        return !sentTrapMine && placePos == null && Bed.getSelfTrapBlock(target, preventEscape.get()) != null;
    }

    private boolean didTrapMine() {
        if (Bed.getSelfTrapBlock(target, preventEscape.get()) == null) return true;
        return BlockHelper.getBlock(stb) == Blocks.AIR || !Automation.isTrapBlock(stb);
    }
