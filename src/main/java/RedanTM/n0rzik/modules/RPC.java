...
public class RPC extends MousModule {

    private final SettingGroup sgGeneral = settings.getDefaultGroup();

    private final Setting<String> title = sgGeneral.add(new StringSetting.Builder().name("title").description("What to display as the RPC's title.").renderer(StarscriptTextBoxRenderer.class).defaultValue("MousClientV0.3").onChanged(booleanSetting -> alertTitle()).build());
    ...
    public RPC() {
        super(Mous.CATEGORY, "RPC", "MousClient RPC for Discord!");
    }
    ...
    public void onActivate() {
        ...
        DiscordIPC.start(1198208926290825247L, null);
        rpc.setStart(DiscordStatus.rpcStart);
        rpc.setLargeImage("mousclient", starscript(title.get()));
        updateDetails();
        messageI = 0;
        messageI2 = 0;
    }
