...
public class EzPop extends MousModule {
    ...
    public EzPop() {
        super(Mous.CATEGORY, "ezpop", "Count player's totem pops.");
        updateSuffixScript();
        updateEzScripts();
        updatePopScripts();
    }
        ...
        if (announceOthers.get() && announceWait <= 1 && mc.player.distanceTo(entity) <= announceRange.get()) {
            ...
            try {
                ...
                if (pmOthers.get()) {
                    ...
                    HelperSurround.messagePlayer(name, StringHelper.stripName(name, popMessage));
                }
        ...
        synchronized (totemPops) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                if (player.deathTime > 0 || player.getHealth() <= 0) {
                    ...
                    if (Ez.currentTargets.contains(player.getEntityName())) Ez.sendAutoEz(player.getEntityName());
                }
            }
        }
    }
